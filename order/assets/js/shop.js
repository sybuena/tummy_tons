...
$(document).ready(function(){
	...
	*/
	...
	$('.qadd').click(function(){
		...
		var id = $(this).attr('id');
		var quantity = $('.details-extra .quantity', $product).val();
		var data = {
			'id' : id,
			'quantity' : quantity
		}
		...
		newItem = '<li>'+
						'<a href="'+$('.name a', $product).prop('href')+'">'+
							'<img src="'+$('.media a img', $product).attr('src')+'" alt="" width="100"/>'+
	...
	$('#nav-basket').hover(function() {
		$('#nav-basket .submenu-preview .product-cart').remove();
		$.ajax({
			url : '/cart/getCart',
			type:'GET',
			dataType:'json',
			success: function(data2) {
				...
				for(i in data2) {
					var newItem = '';
					var image = '/assets/img/spag.jpg';
					if(data2[i].image) {
						image = data2[i].image;
					}
					
		$('.basket-count').html(data2.lenght);
				newItem = '<li class="product-cart">'+
				'<a href="#">'+
				'<img src="'+image+'" alt="" width="100"/>'+
				'<p>'+data2[i].name+'</p>'+
				'<p class="price">'+data2[i].quantity+' x <span class="cur">PHP '+data2[i].price+'</span><span class="total"></span></p>'+											
				'</a>'+
				'<a href="#" id="'+data2[i].cart_ID+'" class="remove-basket-item">remove</a>'+
				'</li>';
					$('#nav-basket .submenu-preview').prepend(newItem);	
				}
			},
